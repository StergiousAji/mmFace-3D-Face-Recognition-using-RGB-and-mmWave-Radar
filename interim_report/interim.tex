...
To illustrate the data acquisition process used thus far, data samples of a single subject is provided in the left half of Figure \ref{fig:rgb_crd_plot}. This grid shows all 15 scenarios laid out by the three conditions along the rows and the five pose variations along the columns. For brevity, the conditions are abbreviated as outlined in Table \ref{tab:conditions}.
...
Work has been conducted in transforming the data collected from the acquisition experiments into suitable formats for input into the proposed models. We are able to successfully transform the Radar bursts into a CRD map which can be plotted showing intensities of received waves into discrete Doppler bins along the $x$-axis and Range bins along the $y$-axis. A data sample of a single CRD frame can be observed in the right half of Figure \ref{fig:rgb_crd_plot}.
