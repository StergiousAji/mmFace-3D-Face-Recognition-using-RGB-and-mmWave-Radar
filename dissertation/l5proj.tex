...
\begin{table}[htbp]
    ...
    \resizebox{1\columnwidth}{!}{
        \begin{tabular}{lS[table-format=2.2]S[table-format=1.4]}
            ...
            Add                  & 81.1 & 0.811 \\
            \rowcolor{lightgreen}
            Hadamard Product  & \cellcolor{lightgreen}91.9 & \cellcolor{lightgreen}0.916 \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/roc_hadamard_product.png}
    \caption{TODO}
    \label{fig:roc_hadamard_product}
\end{figure}

Table \ref{tab:macro_auc} lists the macro-averaged AUC metrics for all nine models. As made evident by the plot and the table, while the Hadamard product offers the highest accuracies it does not maintain its predictive nature across all decision thresholds attaining a relatively low AUC of 0.73. 
...
Taking all metrics into account, it is safe to conclude that the multi-head attention strategy (highlighted in gold) provides the best overall model in both face and liveness identification achieving the third-highest mean accuracy and $F_{0.5}$ measure while maintaining a high AUC beating its above competitors: concatenate and Hadamard product. The one-vs-rest ROC curve for the multi-head attention model is depicted in Figure \ref{fig:roc_multihead_attention}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/roc_multihead_attention.png}
    \caption{TODO}
